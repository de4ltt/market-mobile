...
fun LocalDateTextField(
    ...
    // If the external value changes (e.g., form reset), reflect it.
    ...
    OutlinedTextField(
        ...
        onValueChange = { incoming ->
            ...
            val correctedDigits = clampDayMonth(rawDigits) // keeps year as typed
...
private fun clampDayMonth(digits: String): String {
    ...
    val y = digits.drop(4) // year untouched
...
/**
 * Place cursor after `digitCount` digits inside formatted string "DD.MM.YYYY".
 */
...
private fun parseLocalDate(text: String): LocalDate? {
    ...
    return try {
        val javaDate = JavaLocalDate.of(y, m, d) // validates real calendar
        LocalDate.of(javaDate.year, javaDate.monthValue, javaDate.dayOfMonth)
    } catch (_: Exception) {
        null
    }
}
...
fun ShelfEditDialog(
    ...
    // In your model FullContent default is "Обычный" if null. :contentReference[oaicite:2]{index=2}
    ...
    // Resolve selected StorageLocation from id.
    ...
    // Shelf fields are nullable in the model; enable confirm always, but trim nicely.
    EditDialogScaffold(
...
fun VacationEditDialog(
    ...
    // Uses java.time.LocalDate (same type as in your Vacation model) :contentReference[oaicite:1]{index=1}
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(
                ...
                onClick = {
                    onConfirm(
                        Vacation(
                            // excluded: vacationId, employeeId, approved, reviewed
        ...
        text = {
            Column(
                ...
                // Start date as auto-correcting text field (java.time.LocalDate)
                LocalDateTextFieldJava(
                    label = "Дата начала",
                    value = startDate,
...
private fun DaysSelector(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Slider(
            ...
            valueRange = 1f..28f,
            steps = 26 // 1..28 => 27 intervals => steps = 27-1 = 26
        )
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedButton(
                enabled = days > 1,
...
fun LocalDateTextFieldJava(
    ...
    var tfv by remember { mutableStateOf(TextFieldValue(value?.toDisplayString().orEmpty())) }
    ...
    OutlinedTextField(
        ...
        onValueChange = { incoming ->
            ...
            tfv = TextFieldValue(formatted, selection = TextRange(newCursor))
            onDateChanged(parseJavaLocalDate(formatted))
        },
        label = { Text(label) },
        singleLine = true,
        isError = isError,
...
private fun parseJavaLocalDate(text: String): LocalDate? {
    if (!Regex("""\d{2}\.\d{2}\.\d{4}""").matches(text)) return null
    val (d, m, y) = text.split(".").map { it.toInt() }
    return try {
        LocalDate.of(y, m, d)
